...
def module_count_barchart(scanresult):
    ...
    bar_chart = (
        alt.Chart(counts_df)
        .mark_bar(size=60, cornerRadius=8)
        .encode(
            ...
            color=alt.Color(
                "Category:N",
                scale=alt.Scale(
                    domain=["Python Standard Libraries", "Third-party modules"],
                    range=["#4C78A8", "#F58518"],
                ),
            ),
            tooltip=["Category", "Count"],
        )
        .properties(title="Package Modules Overview", width=400, height=300)
...
def module_distribution_view(scanresult):
    ...
    pie_chart = (
        ...
        .properties(title="Module Composition", width=380, height=380)
    )
    ...
    text = (
        ...
        .encode(text=alt.Text("Count:Q"), color=alt.value("white"))
        .transform_calculate(total="datum.Count")
    )
...
def issue_plot(input_dict):
    ...
    chart = (
        ...
        .properties(title="Overview of Security Weaknesses", width=600, height=600)
    )
...
def complexity_heatmap(scanresult):
    ...
    complexity_slider = alt.param(
        ...
        bind=alt.binding_range(
            min=0, max=max_complexity, step=max(1, max_complexity // 100)
        ),
    )
    ...
    lines_slider = alt.param(
        ...
        bind=alt.binding_range(min=0, max=max_lines, step=max(10, max_lines // 100)),
    )
    ...
    heatmap = (
        ...
        .properties(
            ...
            title=alt.TitleParams(
                ...
                subtitle=["Risk heatmap", f"Based on {total_files} files"],
            ),
        )
    )
    ...
    text = base.mark_text(size=11).encode(
        x="Metric:N",
        y=alt.Y("File:N", sort=alt.SortField(field="RiskScore", order="descending")),
        text=alt.Text("Value:Q", format=".0f"),
        color=alt.condition(highrisk_expr, alt.value("white"), alt.value("black")),
    )
...
def lines_of_code_overview(scanresult, width=800, height=400):
    ...
    df["display_name"] = [
        (
            ...
            if counts.iloc[i] > 1 and row["parent_folder"]
            else row["base_name"]
        )
...
def ast_nodes_overview(scanresult, width=800, height=400):
    ...
    df = (
        df.nlargest(30, "ast_nodes")
        .sort_values("ast_nodes", ascending=False)
        .reset_index(drop=True)
    )
    ...
    text = chart.mark_text(align="left", baseline="middle", dx=5, color="black").encode(
...
def weaknesses_overview(scanresult):
    ...
    if not scanresult or not isinstance(scanresult, dict):
        return (
            alt.Chart(pd.DataFrame({"msg": ["⚠️ No scan result"]}))
            .mark_text()
            .encode(text="msg:N")
        )
    ...
    if not isinstance(file_security_info, dict) or len(file_security_info) == 0:
        return (
            alt.Chart(pd.DataFrame({"msg": ["⚠️ No file security info found"]}))
            .mark_text()
            .encode(text="msg:N")
        )
    ...
    if not counter:
        return (
            alt.Chart(pd.DataFrame({"msg": ["✅ No security weaknesses found."]}))
            .mark_text(size=20)
            .encode(text="msg:N")
        )
    ...
    if df.empty:
        return (
            alt.Chart(pd.DataFrame({"msg": ["⚠️ No security weaknesses found"]}))
            .mark_text(size=20)
            .encode(text="msg:N")
        )
    ...
    if n_constructs == 1:
        chart = (
            ...
            .encode(
                y=alt.Y(
                    "construct:N", sort="-x", title=None, axis=alt.Axis(labelLimit=350)
                ),
                x=alt.X(
                    "count:Q",
                ...
                stroke=alt.condition(
                    alt.datum.is_top5, alt.value("black"), alt.value(None)
                ),
                strokeWidth=alt.condition(
                    alt.datum.is_top5, alt.value(2.5), alt.value(0)
                ),
    ...
    text = (
        alt.Chart(df)
        .mark_text(align="left", dx=5, fontSize=11, color="black")
        .encode(y=alt.Y("construct:N", sort="-x"), x="count:Q", text="count:Q")
    )
    ...
    final_chart = (
        ...
        .configure_axis(grid=False, labelFontSize=12, titleFontSize=13)
    )
...
def sast_files_overview(scanresult):
    ...
    if not records:
        return (
            alt.Chart(pd.DataFrame({"msg": ["✅ No security weaknesses identified."]}))
            .mark_text(size=14)
            .encode(text="msg:N")
        )
    ...
    base_chart = alt.Chart(df).encode(
        ...
        x=alt.X(
            "issues:Q", title="Number of Security Issues", axis=alt.Axis(tickMinStep=1)
        ),
        color=alt.Color(
            "issues:Q",
    ...
    labels = base_chart.mark_text(
        align="left", baseline="middle", dx=8, fontSize=12, fontWeight="bold"
    ...
    chart = (
        ...
        .configure_view(stroke=None)
        .configure_axis(
            grid=True, gridColor="#f0f0f0", labelFontSize=12, titleFontSize=13
        )
    )
...
def weaknesses_radial_overview(scanresult):
        ...
        return (
            alt.Chart(
                pd.DataFrame(
                    {
                        "msg": [
                            "✅ No security weaknesses found. No radial chart created."
                        ]
                    }
                )
            )
    ...
    if len(df) == 1:
        ...
        radius_scale = alt.Scale(
            type="sqrt", zero=True, domain=[0, df["count"].max() * 1.2]
        )
    else:
        df["theta0"] = df["fraction"].cumsum() - df["fraction"]
        df["theta1"] = df["fraction"].cumsum()
